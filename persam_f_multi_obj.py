...
def get_arguments():
    ...
    parser.add_argument("--training_percentage", type=float, default=0.3)
...
def persam_f(args, obj_name, images_path, masks_path, output_path, test_dir):
    ...
    for i in tqdm(range(args.train_epoch_outside)):
        ...
        training_size = int(
            len(os.listdir(os.path.join(images_path, obj_name)))
            * args.training_percentage
        )
        print("Training size: ", training_size)
        for ref_idx in tqdm(range(training_size)):
            # Path preparation
            ref_image_path = os.path.join(
                images_path, obj_name, "{:02}.jpg".format(ref_idx)
            )
            ref_mask_path = os.path.join(
                masks_path, obj_name, "{:02}.png".format(ref_idx)
            )

            if os.path.exists(ref_image_path) is False:
                continue
            ...
            ref_mask = F.interpolate(
                ref_mask, size=ref_feat.shape[0:2], mode="bilinear"
            )
            ...
            if target_feat.sum() == 0 or ref_feat.sum() == 0:
                ref_idx -= 1
                continue
            ...
            optimizer = torch.optim.AdamW(
                mask_weights.parameters(), lr=args.lr, eps=1e-4
            )
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer, args.train_epoch_inside
            )

            for train_idx in tqdm(range(args.train_epoch_inside)):
    ...
    for test_idx in tqdm(range(len(os.listdir(test_images_path)))):
        ...
        if os.path.exists(test_image_path) is False:
            continue
        ...
        mask_colors = np.zeros((256, 256, 3))
        for i in tqdm(range(args.max_objects)):
            ...
            if target_feat.sum() == 0 or test_feat.sum() == 0 or sim.sum() == 0:
                break
        ...
        vis_mask_output_path = os.path.join(
            output_path, f"vis_mask_{test_idx}_objects:{len(history_masks)}.jpg"
        )
